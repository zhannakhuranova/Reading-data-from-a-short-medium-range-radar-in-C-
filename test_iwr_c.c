...
int byteCount = 4000; // frame counter
...
int main()
{
    ...
    DCB dcbSerialParams = {0}; // struct
    dcbSerialParams.DCBlength = sizeof(dcbSerialParams);
    if (!GetCommState(hComm1, &dcbSerialParams)) // winbase.h
    {
    ...
    if (magicOK > 0)
    {
        ...
        for (int tlvIdx = 0; tlvIdx <= numTLVs; tlvIdx++)
        {
            ...
            if (tlv_type == MMWDEMO_UART_MSG_DETECTED_POINTS)
            {
                ...
                int N = 50;
                char text[50];
                for (unsigned long int i = byteCount; i < c; + byteCount)
                {

                    sprintf(text, "file%d.txt", i);
                }
                f1 = fopen(text, "w+");
